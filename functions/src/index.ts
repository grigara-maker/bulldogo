...
/**
 * Odešle fakturu na email uživatele a účetní
 */
async function sendInvoiceEmail(
  ...
  const userEmail = userProfile?.email || paymentData.payerEmail;
  if (!userEmail) {
    functions.logger.warn("No email found for invoice", { orderNumber, userId });
    return;
  }
  ...
  const userMailOptions = {
    ...
    to: userEmail,
    subject: `Faktura ${orderNumber} - BULLDOGO.CZ`,
    html: invoiceHTML,
    text: `Dobrý den ${userName},\n\nDěkujeme za vaši platbu. V příloze naleznete fakturu č. ${orderNumber}.\n\nČástka: ${paymentData.amount} ${paymentData.currency || "CZK"}\nBalíček: ${paymentData.planName || "Balíček"}\n\n© 2025 BULLDOGO.CZ`,
  };

  await smtpTransporter.sendMail(userMailOptions);
  functions.logger.info("✅ Faktura odeslána uživateli", { orderNumber, userEmail });
  ...
  if (accountingEmail && accountingEmail !== userEmail) {
    const accountingMailOptions = {
      ...
      to: accountingEmail,
      subject: `[ÚČETNÍ] Faktura ${orderNumber} - ${userName}`,
      html: invoiceHTML,
      text: `Faktura ${orderNumber} pro ${userName}\n\nČástka: ${paymentData.amount} ${paymentData.currency || "CZK"}\nBalíček: ${paymentData.planName || "Balíček"}\n\n© 2025 BULLDOGO.CZ`,
    };

    await smtpTransporter.sendMail(accountingMailOptions);
    functions.logger.info("✅ Faktura odeslána účetní", { orderNumber, accountingEmail });
  }
}
...
async function activateUserPlan(orderNumber: string): Promise<void> {
  ...
  // Odeslat fakturu na email
  try {
    await sendInvoiceEmail(orderNumber, paymentData, userId);
  } catch (error: any) {
    functions.logger.error("Failed to send invoice email", { orderNumber, error: error?.message });
  }
}
